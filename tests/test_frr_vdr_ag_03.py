"""
Tests for FRR-VDR-AG-03: Do Not Request Extra Info
"""
...
from fedramp_20x_mcp.analyzers.frr.frr_vdr_ag_03 import FRR_VDR_AG_03_Analyzer
from fedramp_20x_mcp.analyzers.base import Severity


# TODO: Implement tests for FRR-VDR-AG-03
# Follow the pattern from test_frr_vdr_08.py and test_frr_ucm_02.py


def test_analyzer_metadata():
    """Test FRR-VDR-AG-03 analyzer metadata."""
    analyzer = FRR_VDR_AG_03_Analyzer()
    
    assert analyzer.FRR_ID == "FRR-VDR-AG-03", "FRR_ID should be FRR-VDR-AG-03"
    assert analyzer.FAMILY == "VDR", "Family should be VDR"
    assert analyzer.FRR_NAME == "Do Not Request Extra Info", "Title mismatch"
    assert analyzer.PRIMARY_KEYWORD == "SHOULD NOT", "Keyword mismatch"
...
def test_metadata():
    ...
    assert analyzer.FRR_ID == "FRR-VDR-AG-03"
    assert analyzer.FAMILY == "VDR"
    assert analyzer.FRR_NAME == "Do Not Request Extra Info"
    assert analyzer.PRIMARY_KEYWORD == "SHOULD NOT"
    assert analyzer.CODE_DETECTABLE == "No", "Agency process requirement should not be code-detectable"
