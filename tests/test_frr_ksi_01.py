"""
Tests for FRR-KSI-01: Application of Key Security Indicators
"""
...
from fedramp_20x_mcp.analyzers.frr.frr_ksi_01 import FRR_KSI_01_Analyzer
from fedramp_20x_mcp.analyzers.base import Severity


# TODO: Implement tests for FRR-KSI-01
# Follow the pattern from test_frr_vdr_08.py and test_frr_ucm_02.py


def test_analyzer_metadata():
    """Test FRR-KSI-01 analyzer metadata."""
    analyzer = FRR_KSI_01_Analyzer()
    
    assert analyzer.FRR_ID == "FRR-KSI-01", "FRR_ID should be FRR-KSI-01"
    assert analyzer.FAMILY == "KSI", "Family should be KSI"
    assert analyzer.FRR_NAME == "Application of Key Security Indicators", "Title mismatch"
...
def test_evidence_collection():
    ...
    assert 'KSI coverage mapping' in queries, "Missing KSI coverage mapping"
    assert 'FedRAMP scope verification' in queries, "Missing scope verification"
    
    artifacts = analyzer.get_evidence_artifacts()
    assert len(artifacts) > 0, "Missing evidence artifacts"
    
    recommendations = analyzer.get_evidence_automation_recommendations()
    assert 'automated_ksi_coverage_mapping' in recommendations, "Missing coverage mapping"
...
def run_all_tests():
    ...
    print("\n" + "=" * 70)
    print(f"Running FRR-KSI-01 Tests ({len(test_functions)} tests)")
    print("=" * 70 + "\n")
